...
En este lab, usaremos servicios y entradas para darle acceso externo a la app de prueba y también para la app de Wordpress.
...
Cambia los valores solicitados en el archivo para que reflejen lo que se haya configurado en el pod nginx pod y aplica el archivo. Si necesitas más información, pide ayuda a tu instructor/a.

Revisa que el servicio se haya iniciado y que esté listado bajo el nombre provisto:
...
Ahora, desde un navegador, intenta acceder al pod nginx pod usando http://\<YOUR VM FQDN OR IP\>:3XXXX (reemplaza 3XXXX por el puerto listado en el comando anterior). Debería abrirse una página web de "Welcome to NGINX".
...
No se realizarán otras acciones con el pod nginx, pero si quieras probar configuraciones adicionales, pide ejemplos a tu instructor/a, o bien o pruébalo tú mismo/a.

## Exponer el pod de Wordpress para que se pueda acceder en internet usando un puerto HTPP predeterminado y una Entrada

Modifica el namespace wordpress-apps usando:
...
* El tipo de servicio debería ser ClusterIP. Esto permitirá que la Entrada (que crearemos a continuación) utilice el servicio.
* Asegúrate de que el selector de pods esté configurado con la misma etiqueta que esté usando el pod de Wordpress.
* Puedes revisar el selector de pods consultando el archivo yml aplica para el pod, o bien usando kubectl describe pod \<PODNAME\>
...
* Puerto del servicio creado antes.

Aplica el archivo wordpress-ingress.yml y revisa que la entrada se haya configurado de manera correcta:

```bash
kubectl get ingress
```

Abre una pestaña en un navegador e intenta ingresar en http://\<WORDPRESS_FQDN\>. Completa la configuración de Wordpress (escribiendo en notepad el nombre de usuario y contraseña configurados) e intenta iniciar sesión en http://\<WORDPRESS_FQDN\>/wp-admin URL.
